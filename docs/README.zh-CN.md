...
| [指标与遥测](metrics-and-telemetry.zh-CN.md) | 订阅限额、会话 token 用量、数据源优先级、隐私、过期状态与测试 |
...
CanvasTTY 目前不加载第三方运行时插件。自定义小组件是随应用一起编译的源码级贡献。这是 MVP 阶段有意为之的设计：Electron 能力采用白名单机制，共享数据保持类型安全，服务商凭据绝不进入渲染进程。
