...
- Terminal card 保持 `54px` header。正常 scale 下，在用户明确重命名前，header 显示 provider mark 与 terminal working directory；custom title 随后替换路径。Close 是唯一 window action，并始终显示在最右侧；canvas card 不提供 maximize/fullscreen。Terminal chrome 不显示 lifecycle dot。
