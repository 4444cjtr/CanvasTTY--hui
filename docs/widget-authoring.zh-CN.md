...
CanvasTTY 的小组件目前是源码级扩展，而不是运行时插件。本指南说明如何在不削弱 Electron 隔离、不虚构遥测数据、不偏离视觉体系的前提下添加小组件。
...
4. 从 [`src/preload/index.ts`](../src/preload/index.ts) 只暴露类型化能力。绝不暴露 Node 原语、原始 IPC、token、cookie 或服务商的原始响应。
...
- 只有当某个界面真的需要滚动或消费滚轮输入时，才把 wheel 事件交给它。被动 Home 小组件必须保留画布缩放。
