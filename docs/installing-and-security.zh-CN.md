...
| macOS arm64（Apple Silicon） | dmg、zip | 两者都包含图形化的 `.app` bundle；`0.8.2` 暂不包含 Intel/x64 构建 |

请只从本仓库的 [GitHub Releases](https://github.com/howdeploy/CanvasTTY/releases) 页面下载产物。`0.8.x` 目前是公共预览版，没有商业代码签名证书，也未经过 Apple 公证（notarization）。因此 Windows SmartScreen 和 macOS Gatekeeper 可能会提示开发者未知。在确认任何警告之前，请先核对 release tag 和产物名称。
