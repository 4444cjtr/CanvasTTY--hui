...
- `TerminalManager` injects the MCP helper per launch for Claude Code and Codex without changing their global configuration. Kimi uses its per-run MCP configuration when supported; older Kimi versions receive a compare-and-swap temporary CanvasTTY entry and one exact permission rule with an atomic recovery journal. Temporary Kimi configuration remains in place until the owning PTY session ends so every provider-created MCP client can initialize. Unrelated MCP entries, credentials, and file/shell permissions are preserved.
