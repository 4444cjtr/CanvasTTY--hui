...
- Wheel input belongs to the surface under the pointer when that surface actually scrolls or consumes wheel input. By default the session list, terminal, and Browser page retain local wheel ownership. Enabling Controls → Zoom over applications explicitly routes terminal and native Browser wheel input to the camera. Passive Home widgets — limits, clock, media, and launcher tiles — allow camera zoom so Home does not shrink the usable zoom area.
