...
- Нижний dock содержит Terminal, Codex, Claude и Kimi. Settings — отдельная плитка. Заготовка встроенного браузера пока намеренно не доступна из HOME.
...
- Settings использует верхнюю полосу General, Appearance, Controls и Plugins. General владеет языком. Appearance — palette, background pattern, shortcut hint, системными HOME tiles и входом в HOME editor. Controls — click focus, hover focus, window snapping, edge panning, zoom sensitivity, wheel direction и keyboard shortcuts. Plugins — install preview, permission review, installed-plugin list, enable/disable/uninstall и contribution actions. Media controls там не появляются.
...
- Selection эксклюзивен: нажатие на пустой canvas снимает его. Выбранный живой терминал удерживает keyboard focus xterm и теряет его при deselect.
- Hover focus — отдельный, выключенный по умолчанию режим selection. После настраиваемой задержки (медленно `500ms`, обычно `250ms`, быстро `80ms`) терминал под указателем выбирается и получает keyboard focus; уход с карточки снимает selection через ту же задержку.
...
- Built-in Browser — movable/resizable core canvas card, а не plugin contribution. Trusted DOM chrome владеет tabs, address/search, back/forward/reload и close; remote page остаётся в sandboxed native `WebContentsView` с отдельным persistent browser profile. Launcher в HOME пока отсутствует. Website permission requests/downloads запрещены до появления явных user-facing policies. Ниже `0.5×`, в Edit HOME и за trusted dialogs native page скрывается, а не перекрывает renderer UI.
...
- Wheel input принадлежит поверхности под указателем, если та действительно scrolls/consumes wheel. Session list и terminal/card surfaces никогда не zoom camera. Passive Home widgets — limits, clock, media, launcher — разрешают camera zoom, чтобы Home не сокращал usable zoom area.
...
- Home, Focus Card, Settings и минимум один живой терминал проверены в настоящем Electron после build.
